    ...
    public class DateTimeFormatAgain
    {
        ...
        public string GetHour(string strDate)
        {
            ...
            if (regTime == true)
            {
                try
                {
                    int minute = Convert.ToInt32(Utilities.RegexFilter(strDate, @"[\d]{1,2}"));
            ...
            if (regTime == true)
            {
                try
                {
                    int hour = Convert.ToInt32(Utilities.RegexFilter(strDate, @"[\d]{1,2}"));
            ...
            if (regTime == true)
            {
                ...
                string apm = Utilities.RegexFilter(strDate, @"(AM|PM)").Trim();
            ...
            if (regTime == true)
            {
                ...
                {
                    time = Utilities.RegexFilter(time, @"[\d]+:[\d]+") + " PM";
                }

                if (time.ToLower().Contains(" sáng"))
                {
                    time = Utilities.RegexFilter(time, @"[\d]+:[\d]+") + " AM";
                }
            }
            ...
            if (time == "") // các kiểu rồi mà vẫn chưa tìm ra time thì thử lấy dạng 12:00 xem sao
            {
                ...
                string am = Utilities.RegexFilter(strDate, @"(:[\d]+)([\s]+)(am)"); // chua dung am
                if (am != "")
                {
                    time = time + " AM";
                }

                string pm = Utilities.RegexFilter(strDate, @"(:[\d]+)([\s]+)(pm)"); // chua dung am
                if (pm != "")
                {
                    time = time + " PM";
                }
        ...
        public string GetDate(string sDate, string ruleDate)
        {
            try
            {
                ...
                {
                    int d = Convert.ToInt32(Utilities.RegexFilter(sDate, @"[\d]+"));
                ...
                {
                    int m = Convert.ToInt32(Utilities.RegexFilter(sDate, @"[\d]+"));
                ...
                {
                    int y = Convert.ToInt32(Utilities.RegexFilter(sDate, @"[\d]+"));
                ...
                if (ruleDate != string.Empty)
                {
                    ...
                    switch (ruleDate)
                    {
                        case "dd/MM": // chi co ngay va thang
                            if (sDate.Length > 5)
                            {
                                dateTmp = Utilities.RegexFilter(dateTmp, @"[\d]+\/[\d]+");
                            }
                        ...
                        case "dd/MM/yy":
                            if (sDate.Length > 8)
                            {
                                dateTmp = Utilities.RegexFilter(dateTmp, @"[\d]+\/[\d]+\/[\d]{2,}");
                            }
                        ...
                        case "ngày dd tháng MM":
                            ...
                            if (sDate.Length > 10)
                            {
                                ...
                                {
                                    List<string> lisDig = Utilities.RegexFilters(dateTmp, @"[\d]+");
        ...
        public string GetDateBySearchText(string sDate, string ruleDate)
        {
            try
            {
                ...
                bool isMinute = Regex.IsMatch(sDate, @"([\d]{1,2} phút trước)|([\d]{1,2} minutes ago)|(cách đây [\d]{1,2} phút)", RegexOptions.IgnoreCase);
                if (isMinute == true)
                {
                    string minute = Utilities.RegexFilter(sDate, @"([\d]{1,2} phút trước)|([\d]{1,2} minutes ago)|(cách đây [\d]{1,2} phút)");
                    int mm = Convert.ToInt32(Utilities.RegexFilter(minute, @"[\d]+"));

                    return DateTime.Now.AddMinutes(-mm).ToString("yyyy-MM-dd");
                }

                // 1 tiếng trước
                bool isHour = Regex.IsMatch(sDate, @"([\d]{1,2} tiếng trước)|([\d]{1,2} giờ trước)|([\d]{1,2} hours trước)|([\d]{1,2} hours ago)|(cách đây [\d]{1,2} giờ)", RegexOptions.IgnoreCase);
                if (isHour == true)
                {
                    string hour = Utilities.RegexFilter(sDate, @"([\d]{1,2} tiếng trước)|([\d]{1,2} giờ trước)|([\d]{1,2} hours trước)|([\d]{1,2} hours ago)|(cách đây [\d]{1,2} giờ)");
                    int hh = Convert.ToInt32(Utilities.RegexFilter(hour, @"[\d]+"));

                    return DateTime.Now.AddHours(-hh).ToString("yyyy-MM-dd");
                }

                bool isAfterDate = Regex.IsMatch(sDate, @"[\d]+ ngày trước", RegexOptions.IgnoreCase);
                if (isAfterDate == true)
                {
                    string d = Utilities.RegexFilter(sDate, @"[\d]+");
                ...
                if (sDate.ToLower().Contains("tháng trước"))
                {
                    ...
                    {
                        string m = Utilities.RegexFilter(sDate, @"[\d]+");
                ...
                if (sDate.ToLower().Contains("năm trước"))
                {
                    ...
                    if (isNamTruoc == true)
                    {
                        string y = Utilities.RegexFilter(sDate, @"[\d]+");
        ...
        public string GetDateByPattern(string sDate, string ruleDate)
        {
            try
            {
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(jan|january|feb|february|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december) [\d]+");
                    if (tmpDate != "")
                    {
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"[\d]+ (january|jan|february|feb|march|mar|april|apr|may|may|june|jun|july|jul|august|aug|september|sep|october|oct|november|nov|december|dec) [\s]{0,}[\d]{4,}");
                    if (tmpDate != "")
                    {
                ...
                if (isMatch == true)
                {
                    string subDate = Utilities.RegexFilter(sDate, @"[\d]+\/[\d]+\.[\d]{4}");

                    List<string> lisDig = Utilities.RegexFilters(subDate, @"[\d]+");
                    if (lisDig.Count == 3)
                    {
                ...
                if (isMatch == true)
                {
                    string subDate = Utilities.RegexFilter(sDate, @"ngày[\s\S]+");
                    List<string> lisDig = Utilities.RegexFilters(subDate, @"[\d]+");
                ...
                if (isMatch == true)
                {
                    List<string> lisDig = Utilities.RegexFilters(sDate, @"[\d]+");
                    if (lisDig.Count >= 3)
                    {
                        return lisDig[2] + "-" + lisDig[1] + "-" + lisDig[0];
                    }
                ...
                if (isMatch == true)
                {
                    ...
                    string tmpDate = Utilities.RegexFilter(sDate, @"[\d]+ (tháng|Tháng) (.*?) [\d]{4}");

                    List<string> lisDig = Utilities.RegexFilters(tmpDate, @"[\d]+");
                    if (lisDig != null && lisDig.Count == 2)
                    {
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(tháng [\d]+ [\d]+, [\d]{4})");
                    if (tmpDate != "")
                    {
                        List<string> lisD = Utilities.RegexFilters(tmpDate, @"[\d]+");
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(thứ )(.*?), [\d]{1,2} (tháng) [\d]{1,2} [\d]{4}|(chủ nhật,) [\d]{1,2} (tháng) [\d]{1,2} [\d]{4}");
                    if (tmpDate != "")
                    {
                        string[] arr = tmpDate.Split(' ');
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(Thứ|Chủ)([\s]+)(.*?)(,)([\s]+)([\d]+)([\s]+)(Tháng)([\s]+)([\d]+)([\s]+)([\d]{4})");
                    if (tmpDate != "")
                    {
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(NGÀY )[\d]+( THÁNG )[\d]+(, )[\d]+").Replace(",", "").Trim();
                    if (tmpDate != "")
                    {
                        List<string> ls = Utilities.RegexFilters(tmpDate, @"[\d]+");
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"[\d]+ Th[\d]+ [\d]{4}").Trim();
                    if (tmpDate != "")
                    {
                        List<string> ls = Utilities.RegexFilters(tmpDate, @"[\d]+");
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"([\d]+:[\d]+ - [\d]{1,2}\/[\d]{1,2})$");
                    if (tmpDate != "")
                    {
                        string[] arr = sDate.Split('-');
                        if (arr.Length > 0)
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"([\d]+):([\d]+) (AM|PM), ([\d]+) ([\d]+) ([\d]{4})");
                    if (tmpDate != "")
                    {
                        string[] arr = sDate.Split(' ');
                        if (arr.Length > 0)
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"[\d]+[\s]+Th[\d]+[\s]+[\d]{4} ");
                    if (tmpDate != "")
                    {
                        string[] arr = sDate.Split(' ');
                        if (arr.Length > 0)
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"[\d]+ tháng ([\w\W]+), [\d]{4,}").ToLower();
                    if (tmpDate != "")
                    {
                        string year = Utilities.RegexFilter(tmpDate, @"[\d]{4,}$");
                        string day = Utilities.RegexFilter(tmpDate, @"^[\d]+");
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"T[\d]+, [\d]+ \/ [\d]+");
                    if (tmpDate != "")
                    {
                        string[] arr = sDate.Split(' ');
                        if (arr.Length > 3)
                        {
                            string day = Utilities.RegexFilter(arr[0], @"[\d]+");
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"[\d]+(th|st|nd|rd), Tháng [\w\W]+, [\d]{4}");
                    if (tmpDate != "")
                    {
                        string[] arr = tmpDate.Split(',');
                        if (arr.Length > 2)
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(Tháng) [\w\W]+ [\d]+, [\d]{4}");
                    if (tmpDate != "")
                    {
                        //Tháng Chín 10, 2016
                        string day = Utilities.RegexFilter(tmpDate, @"[\d]+");
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(jan|january|feb|february|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december) [\d]+, [\d]{4}");
                    if (tmpDate != "")
                    {
                        string[] arr = tmpDate.Split(' ');
                        if (arr.Length > 2)
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"[\d]+[\s]+(jan|january|feb|february|mar|march|apr|april|may|jun|june|jul|july|aug|august|sep|september|oct|october|nov|november|dec|december)[\s]+[\d]{2,}");
                    if (tmpDate != "")
                    {
                        string[] arr = tmpDate.Split(' ');
                        if (arr.Length > 2)
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(July|January|February|March|April|May|June|August|September|October|November|December)[\s]+[\d]{1,}public int \s]+[\d]{4}");
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[\s]+[\d]+[\s]+[\d]{4}");
                    if (tmpDate != "")
                    {
                        string[] arr = tmpDate.Split(' ');
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"[\d]+ (january|jan|february|feb|march|mar|april|apr|may|may|june|jun|july|jul|august|aug|september|sep|october|oct|november|nov|december|dec)public int \s]{0,}[\d]{4,}");
                    if (tmpDate != "")
                    {
                        string[] arr = tmpDate.Split(' ');
                        if (arr.Length > 2)
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec) [\d]+(th|nd|st|rd), [\d]+ [\d]+:[\d]+ (am|pm)");
                    if (tmpDate != "")
                    {
                        string year = Utilities.RegexFilter(tmpDate, @"[\d]{4,}");
                        string day = Utilities.RegexFilter(tmpDate, @"[\d]+");
                        string month = Utilities.RegexFilter(tmpDate, "(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)");
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec) [\d]+(th|nd|st|rd), [\d]+:[\d]+ (am|pm)");
                    if (tmpDate != "")
                    {
                ...
                if (isMatch == true)
                {
                    string tmpDate = Utilities.RegexFilter(sDate, @"(monday|tuesday|wednesday|thursday|friday|saturday|sunday), [\d]+:[\d]+ (am|pm)");
                    if (tmpDate != "")
                    {
                        string strDay = Utilities.RegexFilter(tmpDate, "(monday|tuesday|wednesday|thursday|friday|saturday|sunday)");
        ...
        public string FindDateFromText(string text)
        {
            ...
            if (isMatch == true)
            {
                ...
                if (text.Contains("giây") || text.Contains("second"))
                {
                    ...
                    try
                    {
                        string tt = Utilities.RegexFilter(text, @"[\d]+");
